...
@Injectable()
export class CirclecrmTimezoneService {

    protected baseUrl: string;

    private cache: ITimezone[] = [];

    public constructor(@Inject(CONTACTS_CONFIG) configToken: any, protected http: HttpClient) {
        this.baseUrl = configToken as string;
    }

    public refresh(background?: boolean): Promise<ITimezone[]> {
        this.cache = [];
        return this.doGetAll(background).toPromise();
    }

    public getAll(background?: boolean): Promise<ITimezone[]> | ITimezone[] {
    ...
    protected doGetAll(background?: boolean): Observable<ITimezone[]> {

        return this.http.get(merge_url_fragments(this.baseUrl, 'utils/timezones'),
            {observe: 'response', responseType: 'text', headers: create_headers(background)})
            .map((res: HttpResponse<string>) => {
                const result = json_decode<ITimezone[]>(res.body || '');
                this.cache = result || [];
                return this.cache;
            });
    }
